"""
STEP 6C: ENTRAÎNEMENT WIDE+DEEP - VERSION PURE
Wide = 32 Excel features uniquement
Deep = Signal → CNN+Transformer → 64 features
Total = 96 features (32 + 64), PAS de redondance
"""
...
print("="*80)
print("STEP 6C: ENTRAÎNEMENT WIDE+DEEP - VERSION PURE")
print("="*80)
print(f"\n💡 Architecture:")
...
class WideDeepModel(nn.Module):
    def __init__(self, num_wide_features=32, num_classes=5):
        ...
        self.fusion = nn.Sequential(
            nn.Linear(64 + 32, 128),  # 64 deep + 32 wide = 96
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(128, 64),
            nn.ReLU(),
    ...
    def forward(self, X_signal, W_features):
        ...
        # Wide branch
        wide_features = self.wide_fc(W_features)  # (batch, 32)
        
        # Fusion
        combined = torch.cat([deep_features, wide_features], dim=1)  # (batch, 96)
...
if __name__ == '__main__':
    ...
    model = WideDeepModel(num_wide_features=32, num_classes=NUM_CLASSES).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_auc_macro > best_val_auc:
            ...
            torch.save(model.state_dict(), 'model_wide_deep_pure.pth')
    ...
    model.load_state_dict(torch.load('model_wide_deep_pure.pth'))
    test_loss, test_auc_macro, test_auc_micro = evaluate(model, test_loader, criterion)
    
    print(f"\n{'='*80}")
    print("RÉSULTATS FINAUX - WIDE+DEEP PURE")
    print(f"{'='*80}")
    ...
    with open('history_pure.json', 'w') as f:
        json.dump(history, f, indent=2)
    
    print(f"\n✅ STEP 6C TERMINÉ")
    print(f"  • Modèle: model_wide_deep_pure.pth")
    print(f"  • Historique: history_pure.json")
